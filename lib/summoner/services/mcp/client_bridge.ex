defmodule Summoner.Services.MCP.ClientBridge do
  @moduledoc """
  ...
  defp start_child(context_workspace_id, server, anubis_opts) do
    case DynamicSupervisor.start_child(
           ...
           {Summoner.Services.MCP.ClientWrapper, {server, anubis_opts}}
         ) do
      {:ok, _sup_pid} ->
  ...
  defp build_transport(context_workspace_id, %McpServer{transport: :stdio} = server) do
    workspace_dir = Workspaces.workspace_dir(context_workspace_id)
