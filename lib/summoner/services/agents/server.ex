defmodule Summoner.Services.Agents.Server do
  @moduledoc """
  ...
  alias Summoner.Services.Orchestration.{
    BuiltinTools,
  ...
  def init(opts) do
    ...
    Events.subscribe({:agent_config, agent.id})
  ...
  @impl true
  def handle_info(%Summoner.Domain.Events.AgentConfigChanged{}, state) do
  ...
  defp load_workspace_settings(workspace_id) do
    Workspaces.get_settings!(workspace_id)
  end
  ...
  defp load_media_tool(agent, type, definition_fn) do
    case MediaProviders.resolve_media_provider(agent, type) do
      nil -> []
