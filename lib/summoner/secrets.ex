  ...
  def delete_secret(%{user: _user}, %Secret{} = secret) do
    ...
    scope_query =
      if secret.workspace_id do
        where(Summoner.MCP.McpServer, [s], s.workspace_id == ^secret.workspace_id)
      else
        where(Summoner.MCP.McpServer, [s], s.tenant_id == ^secret.tenant_id)
      end
