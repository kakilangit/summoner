  ...
  def mount(params, _session, socket) do
    ...
    if socket.assigns.tenant_can?.(:manage_resources) do
      {server, title} =
        case params["id"] do
          nil ->
            {%McpServer{tenant_id: tenant.id}, "New Rune"}

          id ->
            {MCP.get_tenant_server!(tenant.id, id), "Edit Rune"}
        end
      ...
      socket =
        ...
        |> assign(
          breadcrumbs: [
            ...
            {"Runes", ~p"/realms/#{tenant.id}/runes"},
            {title, nil}
          ]
        )
    ...
    case MCP.create_server(socket.assigns.current_scope, params) do
      {:ok, _server} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/realms/#{tenant.id}/runes")}
    ...
    <div class="max-w-lg mx-auto space-y-6">
      ...
      >
        ...
        <div class="flex items-center gap-4">
          <.link
            navigate={~p"/realms/#{@tenant.id}/runes"}
