  ...
  def mount(params, _session, socket) do
    ...
    if socket.assigns.tenant_can?.(:manage_resources) do
      {provider, title} =
        case params["id"] do
          nil ->
            {%Provider{tenant_id: tenant.id}, "Add Gateway"}

          id ->
            {Providers.get_tenant_provider!(tenant.id, id), "Edit Gateway"}
        end

      changeset = Provider.changeset(provider, %{})

      secrets = Secrets.list_tenant_secrets(tenant.id)
      ...
      socket =
        ...
        |> assign(
          breadcrumbs: [
            ...
            {"Gateways", ~p"/realms/#{tenant.id}/gateways"},
            {title, nil}
          ]
        )
    ...
    case Providers.create_provider(socket.assigns.current_scope, params) do
      {:ok, _provider} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/realms/#{tenant.id}/gateways")}
    ...
    <div class="max-w-lg mx-auto space-y-6">
      ...
      <.form for={@form} id="provider-form" phx-change="validate" phx-submit="save" class="space-y-4">
        ...
        <div class="flex items-center gap-4">
          <.link
            navigate={~p"/realms/#{@tenant.id}/gateways"}
