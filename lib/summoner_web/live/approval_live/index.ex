defmodule SummonerWeb.ApprovalLive.Index do
  ...
  @sort_options [{"Name", :name}, {"Trigger", :trigger_type}]
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      socket
      |> assign(
        page_title: "Rites - #{workspace.name}",
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    authorize(socket, :configure, fn ->
      ...
      workspace = socket.assigns.workspace
      rule = Approvals.get_rule!(scope, workspace.id, id)

      case Approvals.delete_rule(scope, rule) do
        {:ok, _} ->
          {:noreply, socket |> load_page() |> put_flash(:info, "Rite removed.")}

        {:error, _} ->
          {:noreply, put_flash(socket, :error, "Could not remove rite.")}
      end
    end)
  end
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold">Rites</h1>
        <.link
          :if={@can?.(:configure)}
          navigate={
            ~p"/tenants/#{@workspace.tenant_id}/workspaces/#{@workspace.id}/approval-rules/new"
          }
      ...
      <.list_controls
        ...
        sort_dir={@sort_dir}
        sort_options={@sort_options}
        placeholder="Search rites..."
      />

      <div :if={@page.entries == []} class="text-center py-12 text-base-content/60">
        <p :if={@filter == ""}>
          No rites defined. Create approval rules to require human review for agent actions.
        </p>
        <p :if={@filter != ""}>No rites match your search.</p>
      </div>
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              <span class="badge badge-ghost badge-xs">{rule.trigger_type}</span>
          ...
          <div class="flex gap-2 flex-shrink-0">
            <.link
              :if={@can?.(:configure)}
              navigate={
                ~p"/tenants/#{@workspace.tenant_id}/workspaces/#{@workspace.id}/approval-rules/#{rule.id}/edit"
              }
            ...
            <.confirm_modal
              ...
              title="Delete rite?"
              message="Agents will no longer require approval for actions matching this rule."
              confirm_text="Delete"
  ...
  defp load_page(socket) do
    ...
    page = Approvals.list_rules_paginated(scope, workspace.id, opts)
