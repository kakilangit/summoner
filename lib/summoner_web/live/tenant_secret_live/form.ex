  ...
  def mount(params, _session, socket) do
    ...
    if socket.assigns.tenant_can?.(:manage_resources) do
      {secret, title} =
        case params["id"] do
          nil ->
            {%Secret{tenant_id: tenant.id}, "New Seal"}

          id ->
            {Secrets.get_tenant_secret!(tenant.id, id), "Edit Seal"}
        end
      ...
      socket =
        ...
        |> assign(
          breadcrumbs: [
            ...
            {"Seals", ~p"/realms/#{tenant.id}/seals"},
            {title, nil}
          ]
        )
    ...
    case Secrets.create_secret(socket.assigns.current_scope, params) do
      {:ok, _secret} ->
        {:noreply,
         ...
         |> push_navigate(to: ~p"/realms/#{tenant.id}/seals")}
    ...
    <div class="max-w-lg mx-auto space-y-6">
      ...
      >
        ...
        <div class="flex items-center gap-4">
          <.link
            navigate={~p"/realms/#{@tenant.id}/seals"}
