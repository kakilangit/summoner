defmodule SummonerWeb.ProviderLiveTest do
  ...
  describe "Index" do
    test "lists providers", %{conn: conn, scope: scope, workspace: ws} do
      ...
      {:ok, _view, html} = live(conn, ~p"/tenants/#{ws.tenant_id}/workspaces/#{ws.id}/providers")
    ...
    test "deletes a provider", %{conn: conn, scope: scope, workspace: ws} do
      ...
      {:ok, view, _html} = live(conn, ~p"/tenants/#{ws.tenant_id}/workspaces/#{ws.id}/providers")
  ...
  describe "Form - New" do
    test "creates a provider", %{conn: conn, workspace: ws} do
      {:ok, view, _html} =
        live(conn, ~p"/tenants/#{ws.tenant_id}/workspaces/#{ws.id}/providers/new")
  ...
  describe "Form - Edit" do
    test "updates a provider", %{conn: conn, scope: scope, workspace: ws} do
      ...
      {:ok, view, html} =
        live(conn, ~p"/tenants/#{ws.tenant_id}/workspaces/#{ws.id}/providers/#{provider.id}/edit")
